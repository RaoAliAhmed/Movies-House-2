...
export default function Movies({ movies, genres }) {
  ...
  const filteredMovies = searchQuery
    ? filteredByGenre.filter(m => m.title.toLowerCase().includes(searchQuery.toLowerCase()))  // Filter by title
    : filteredByGenre;
  ...
  return (
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <h1 className="text-3xl font-bold mb-6 text-center text-white">All Movies</h1>
      ...
      {filteredMovies.length === 0 ? (
        <p className="text-center text-white">No movies found matching the search criteria.</p>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredMovies.map(movie => (
            <div key={movie.id} className="bg-white p-5 rounded-xl shadow-md hover:shadow-xl transition">
